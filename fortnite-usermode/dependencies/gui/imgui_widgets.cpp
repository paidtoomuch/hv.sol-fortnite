...
int ImParseFormatPrecision(const char* fmt, int default_precision)
{
    ...
    if (*fmt == 'e' || *fmt == '_') // Maximum precision with scientific notation
        precision = -1;
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    if (apply_named_filters && (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase | ImGuiInputTextFlags_CharsNoBlank | ImGuiInputTextFlags_CharsScientific)))
    {
        ...
        if (flags & ImGuiInputTextFlags_CharsScientific)
            if (!(c >= '0' && c <= '9') && (c != c_decimal_point) && (c != '-') && (c != '+') && (c != '*') && (c != '/') && (c != 'e') && (c != '_'))
                return false;
...
bool ImGui::ColorEdit3(float spacing, const char* label, float col[3], ImGuiColorEditFlags flags)
{
    return ColorEdit4(spacing, label, col, flags | ImGuiColorEditFlags_NoAlpha);
}
...
bool ImGui::ColorEdit4(float spacing, const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton(spacing, "##ColorButton", col_v4, flags))
        {
        ...
        if (BeginPopup("picker"))
        {
            ...
            value_changed |= ColorPicker4(spacing, "##picker", col, picker_flags, &g.ColorPickerRef.x);
            EndPopup();
        }
    }
...
bool ImGui::ColorPicker3(float spacing, const char* label, float col[3], ImGuiColorEditFlags flags)
{
    float col4[4] = { col[0], col[1], col[2], 1.0f };
    if (!ColorPicker4(spacing, label, col4, flags | ImGuiColorEditFlags_NoAlpha))
        return false;
...
bool ImGui::ColorPicker4(float spacing, const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (!(flags & ImGuiColorEditFlags_NoOptions))
        ColorPickerOptionsPopup(spacing, col, flags);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ColorButton(spacing, "##current", col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2));
        if (ref_col != NULL)
        {
            Text("Original");
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
            if (ColorButton(spacing, "##original", ref_col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2)))
            {
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            if (ColorEdit4(spacing, "##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(spacing, "##hsv", col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(spacing, "##hex", col, sub_flags | ImGuiColorEditFlags_DisplayHex);
        PopItemWidth();
    }
...
bool ImGui::ColorButton(float spacing, const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    // 50 is main for spacing 
    bb.Min.x += spacing, bb.Min.y += 5, bb.Max.x += spacing, bb.Max.y -= 5; // (4 is perfect) bottom half, bigger number higher goes up and opposite will make longer

    ImRect bb_w(window->DC.CursorPos, window->DC.CursorPos + size);
    ...
    {
       float mid_x = IM_ROUND((bb_w.Min.x + bb_w.Max.x) * 0.5f);
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_w.Min.x + grid_step, bb_w.Min.y), bb_w.Max, GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), rounding, ImDrawFlags_RoundCornersRight);
        window->DrawList->AddRectFilled(bb_w.Min, ImVec2(mid_x, bb_w.Max.y), GetColorU32(col_rgb_without_alpha), rounding, ImDrawFlags_RoundCornersLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb.Min, bb.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
        else
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) // if dropped down
        {
            if (g.Style.FrameBorderSize > 0.0f)
                RenderFrameBorder(bb_w.Min, bb_w.Max, rounding);
            else
                window->DrawList->AddRect(bb_w.Min, bb_w.Max, GetColorU32(ImGuiCol_FrameBg), rounding); // Color button are often in need of some sort of border
        }
        else
        {
            if (g.Style.FrameBorderSize > 0.0f)
    ...
    if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropSource())
    {
        ...
        ColorButton(spacing, desc_id, col, flags);
        SameLine();
        TextEx("Color");
        EndDragDropSource();
    }
    ...
    if (!(flags & ImGuiColorEditFlags_NoTooltip) && hovered)
        ColorTooltip(spacing, desc_id, &col.x, flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf));
...
void ImGui::ColorTooltip(float spacing, const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ImGuiContext& g = *GImGui;
    ...
    ColorButton(spacing, "##preview", cf, (flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
    SameLine();
...
void ImGui::ColorPickerOptionsPopup(float spacing, const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            ColorPicker4(spacing, "##previewing_picker", &previewing_ref_col.x, picker_flags);
            PopID();
        }
        PopItemWidth();
    }
