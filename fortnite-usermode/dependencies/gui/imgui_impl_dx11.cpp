...
// very temp till i find another way of d3dcompile
typedef HRESULT(__stdcall* tD3DCompile)(LPCVOID pSrcData, SIZE_T SrcDataSize, LPCSTR pSourceName, const D3D10_SHADER_MACRO* pDefines, ID3D10Include* pInclude, LPCSTR pEntrypoint, LPCSTR pTarget, UINT Flags1, UINT Flags2, ID3D10Blob** ppCode, ID3D10Blob* ppErrorMsgs);
tD3DCompile oD3DCompile = 0;
...
void get_dec_str(unsigned char* str, size_t len, unsigned int val)
{
    ...
    //str[i - 1] = '\0'; // do not terminate string in this case, just replacing the "%i"
}
...
HMODULE GetD3DCompiler()
{
    ...
    for (int i = 30; i < 50; i++)
    {
        ...
        if (GetModuleHandle(gConstructedString)) return GetModuleHandle(gConstructedString);
    }
    return NULL;
}
...
bool InitImports()
{
    char LocalString[] = "D3DCompile";
    oD3DCompile = (tD3DCompile)GetProcAddress(GetD3DCompiler(), LocalString);
    if (oD3DCompile) return true;
    else return false;
}
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        // fuck d3dcompiler_43.dll
        ID3DBlob* vertexShaderBlob; 
       // if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_4_0", 0, 0, &vertexShaderBlob, NULL))) {
        
        if (!InitImports()) {
            printf(" [FAILED] -> InitImports.\n");
        }

        //D3DWriteToBlob(&vertexShaderBlob, L"", true);

        if (FAILED(oD3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_4_0", 0, 0, &vertexShaderBlob, NULL))) {
            printf(" [FAILED] -> D3DCompile.\n");
        ...
        {
            printf(" [FAILED] -> CreateVertexShader.\n");
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(oD3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_4_0", 0, 0, &pixelShaderBlob, NULL)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
