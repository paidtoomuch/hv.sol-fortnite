...
{
	//printf(_("EPIC FAIL -> FILE:%s LINE:%d CODE=(int)%d\n"), file, line, (int)code);
	throw std::runtime_error( TEXT( "EPIC FAIL" ) );
}
...
inline VOID xAdjustPrivilege( LPCWSTR PrivilegeName, BOOL Enable )
{
...
inline DWORD xGetPidByName( LPCWSTR name )
{
	DWORD Pid = 0;
...
inline PVOID xFindPattern( HANDLE Process, LPVOID ImageBase, const std::vector<uint8_t>& Pattern )
{
...
inline void xProtectwrite_virtual( HANDLE Process, const std::vector<uint8_t>& OrigBytes, const std::vector<uint8_t>& PatchBytes, PVOID Addr, SIZE_T Offset )
{
...
inline PVOID xPatchProcess( LPCWSTR ProcName, const std::vector<uint8_t>& OrigBytes, const std::vector<uint8_t>& PatchBytes, PVOID PatchAddr, SIZE_T PatchOffset )
{
	...
	if ( ProcName ) {
		...
		xAdjustPrivilege( TEXT( L"SeDebugPrivilege" ), TRUE );
	...
	if ( PatchAddr ) {
		xProtectwrite_virtual( *Process, OrigBytes, PatchBytes, PatchAddr, PatchOffset );
		Result = PatchAddr;
	}
	else {
		...
		do {
			if ( Entry.th32ProcessID == ProcId ) {
				...
				if ( Addr ) {
					xProtectwrite_virtual( *Process, OrigBytes, PatchBytes, Addr, PatchOffset );
					Result = Addr;
					break;
				}
			}
...
inline std::vector<uint8_t> xParseByteArray( LPCSTR Str )
{
	XGUARD( Str );
